...
testEachModel("has correct size", async ({ modelJsonUrl, width, height }) => {
...
testEachModel(
    ...
    async ({ cubismVersion, modelJsonUrl, width, height }) => {
            ...
            listeners: {
                settingsJSONLoaded(json) {
                    if (cubismVersion === 2) {
                        ...
                        (json as CubismSpec.ModelJSON).Layout = Object.fromEntries(
